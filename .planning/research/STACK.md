...
**Domain:** AI agent civilization simulator (TypeScript CLI package)
...
These are already available via `@genesis/shared` and `@genesis/engine` dependencies. LINEAGE imports them, it does not declare its own versions. Listed here for completeness and version pinning awareness.
...
Genesis uses `"noEmit": true` in tsconfig and raw `.ts` source exports (`"main": "./src/index.ts"`). There is no build step -- packages import TypeScript directly using Node's ESM + TypeScript module resolution (`"moduleResolution": "nodenext"`). LINEAGE follows the same pattern.

**No tsup, tsdown, or any bundler needed.** The package is consumed within a pnpm workspace where TypeScript sources are imported directly. A build step would only be needed if publishing to npm as a standalone package, which is out of scope for v1.

## Installation

```bash
# In lineage/ project root (or as workspace package)
pnpm add @anthropic-ai/claude-agent-sdk commander chalk ora cli-table3 log-update

# Dev dependencies (inherited from Genesis root, but declare if standalone)
pnpm add -D typescript @types/node vitest tsx
```

If operating as a pnpm workspace package within Genesis, most dev dependencies come from the root `package.json`. The simulation package declares only its own runtime dependencies.
...
**If standalone operation (current v1):**
...
**If workspace package in Genesis monorepo (combination target):**
...
**If adding Genesis Interpreter integration later (v2+):**
...
The core execution pattern for LINEAGE citizen agents:

```typescript
import { query } from "@anthropic-ai/claude-agent-sdk";

// Each citizen is a query() call with role-specific system prompt
for await (const message of query({
  prompt: citizenPrompt,          // Inheritance + seed problem + role context
  options: {
    systemPrompt: roleSystemPrompt,  // Builder/Skeptic/Archivist/etc.
    maxTurns: calculateMaxTurns(deathProfile),  // Mortality = limited turns
    allowedTools: [],               // Citizens think, they don't use tools
    permissionMode: "dontAsk",      // Headless execution, no human prompts
    cwd: simulationDir,             // Isolated working directory
    model: "sonnet",                // Cost-effective for many agents
  }
})) {
  // Stream citizen output, track context usage for death profiles
  if (message.type === "assistant") {
    trackContextUsage(citizen, message);
    checkDeathConditions(citizen, message);
  }
}
```
...
---
*Stack research for: LINEAGE AI agent civilization simulator*
*Researched: 2026-03-24*
